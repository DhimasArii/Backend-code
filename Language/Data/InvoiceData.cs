...
namespace Language.Data
{
    public class InvoiceData
    {
        ...
        //GetAllByUserId
        public List<Invoice> GetAllByUserId(Guid user_id)
        {
            List<Invoice> invoices = new List<Invoice>();
        ...
        public bool CreateInvoice(Invoice invoice, Detail_Invoice detail_Invoice, Guid checkout_id)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                try
                {
                    string invoiceNumber = GenerateInvoiceNumber(invoice.user_id);
        ...
        //create single invoice for buy now
        public bool CreateSingleInvoice(Invoice invoice, Detail_Invoice detail_Invoice)
        {
            bool result = false;
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    command2.CommandText = @"INSERT INTO detail_invoice (detail_invoice_id, invoice_id, schedule_id)
                                            VALUES (@detail_invoice_id, @invoice_id,@schedule_id)";

                    command2.Parameters.AddWithValue("@detail_invoice_id", detail_Invoice.detail_invoice_id);
                    command2.Parameters.AddWithValue("@invoice_id", invoice.invoice_id);
                    command2.Parameters.AddWithValue("@schedule_id", detail_Invoice.schedule_id);
        ...
        private string GenerateInvoiceNumber(Guid user_id)
        {
            string prefix = "DLA";
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                command.CommandText = "SELECT MAX(invoice_number) FROM invoice WHERE invoice_number LIKE 'DLA%' AND user_id = @user_id";
